	...
	if (strlen($cuilReg)==11) {
		...
		if ($sentencia = $mysqli->prepare($sql)) {
			...
			if ($cant != 0) {
				...
				$descrierror = "Error cuil existente";
				return array(2,$descrierror);
			}
		}
	} else {
		print ("Error cuil del registro numero: $numReg <br/>\n");
		$descrierror = "Error cuil invalido";
		return array(2,$descrierror);
	}
...
if ($nomArcOK==0) {
	...
	$errores[$i] = array("registro" => 0, "coderror" => $nomArcOK, "descrierror" => "Error nombre de archivo");
}
